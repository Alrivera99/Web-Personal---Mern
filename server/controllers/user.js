...
function signUp(req, res){
    ...
    if(!password || !repeatPassword){   
        res.status(404).send({message: "Las contasenas son obligatorias"});
    }else{
        if(password !== repeatPassword){
        res.status(404).send({message: "Las contasenas tienen que ser iguales"});
        }else{
        bcrypt.hash(password, null, null, function(err, hash){
            if(err){
                res
                .status(500)
                .send({message: "Error al encriptar la contrasena"});
            } else{
                user.password = hash;
...
async function updateUser(req, res){
    ...
    if(userData.password){
      await bcrypt.hash(userData.password, null, null, (err, hash) =>{
            if(err){
                res.status(500).send({message: "Error al encriptar la contra."})
            }else{
        ...
        if(err){
            ...
            if(!userStored){
                ...
                if(active === true){
                    ...
                    res.status(200).send({message: "El usuario desactivado correctamente"})
                }
            }
        }
    })
